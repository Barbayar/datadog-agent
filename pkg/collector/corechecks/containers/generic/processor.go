...
func (p *Processor) Run(sender aggregator.Sender, cacheValidity time.Duration) error {
	allContainers, err := p.lister.List()
	if err != nil {
		return fmt.Errorf("cannot list containers from metadata store, containers metrics will be missing, err: %w", err)
	}
	...
	getCollector := func(runtime workloadmeta.ContainerRuntime) metrics.Collector {
		...
		collector := p.provider.GetCollector(string(runtime))
		if collector != nil {
	...
	for _, container := range allContainers {
		...
		if p.filter.IsExcluded(container.Name, container.Image.Name, container.Labels["io.kubernetes.pod.namespace"]) {
			continue
		}
		...
		tags = p.adapter.AdaptTags(tags, container)
...
func (p *Processor) sendMetric(senderFunc func(string, float64, string, []string), metricName string, value *float64, tags []string) {
	...
	metricName, val := p.adapter.AdaptMetrics(metricName, *value)
