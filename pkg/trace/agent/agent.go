...
func (a *Agent) Process(p *api.Payload) {
	...
	for i := 0; i < p.Traces.Len(); i++ {
		...
		tracen := int64(len(t.GetSpans()))
		atomic.AddInt64(&ts.SpansReceived, tracen)
		err := normalizeTrace(p.Source, t.GetSpans())
		if err != nil {
		...
		root := traceutil.GetRoot(t.GetSpans())
		...
		if !p.ClientComputedTopLevel {
			...
			traceutil.ComputeTopLevel(t.GetSpans())
		}

		env := a.conf.DefaultEnv
		if v := traceutil.GetEnv(t.GetSpans()); v != "" {
			// this trace has a user defined env.
			env = v
		}
		pt := ProcessedTrace{
			Trace:            t.GetSpans(),
			WeightedTrace:    stats.NewWeightedTrace(t.GetSpans(), root),
			Root:             root,
			Env:              env,
		...
		if !p.ClientComputedStats {
			if envtraces == nil {
				envtraces = make([]stats.EnvTrace, 0, p.Traces.Len())
			}
		...
		if keep {
			...
			ss.Size += t.Msgsize()
			ss.SpanCount += int64(len(t.GetSpans()))
		}
		if len(events) > 0 {
